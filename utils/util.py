...
def compact_name(value, max_len=48):
    text = str(value)
...
def get_results_save_path(lr, lam, num_feature, m_choice):
...
def get_save_parameter(lr, lam, num_feature, run, tau, args):
    save_path = get_results_save_path(lr, lam, num_feature, args.m_choice)
    os.makedirs(save_path, exist_ok=True)

    layers = len(args.hidden_features)
    compact_model_name = compact_name(args.model_name)
    save_res_fname = f'{save_path}/{compact_model_name}_layers_{layers}_{args.dataset_name}_tau_{tau}_run_{run}'

    args.save_model_name = f'{compact_model_name}_seed_{args.seed + run}'
    save_model_folder = os.path.join(".", "saved_models", args.dataset_name, args.save_model_name)
